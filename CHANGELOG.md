...
- 恢复助手新增有盘符的可移动存储卡/U 盘直接只读扫描，复用 JPG、RAW 与 MP4 结构识别；独立提权读取进程拒绝系统盘和来源盘输出，不写入原卡。
- 恢复输出自动使用 `D:\HanabeRecovery\日期\时间_来源_任务编号`，展示实际位置，导出前检查空间与目录重定向；取消或设备断开后需重新扫描。
- 当前直接设备入口不支持无盘符设备、固定 USB 硬盘、丢失分区或碎片重组；设备候选需导出后查看画面。真实存储卡及 UAC 流程仍待实机验证。
