...
- ダークなチェック表示の複数選択、表示比率が分かるスクロール進捗、最小ウィンドウサイズで保護された3ペインレイアウト
...
- 実画像上でウォーターマークを直接ドラッグでき、サイズ変更時も署名の中心位置を維持。高密度タイル配置にも対応
- 縦横比が異なる画像の余白を、現在画像のぼかしガラス背景で埋めるオプション
...
### 🎬 動画プレビュー
- LibVLC ベースのローカル動画再生。エンジンをバックグラウンドで事前初期化し、プレーヤーを再利用して起動待ちを短縮
- ハードウェアデコード、ローカルファイルキャッシュ、遅延フレーム処理、シーク、再生速度、全画面に対応
...
- **テスト駆動**：647 件の自動テストがプロジェクトを守る（デザインシステム回帰テストが、ハードコードされた色や仕様外の角丸など UI の違反項目を自動検出）
- **イテレーションの流れ**：20% → 60% → 70% → 機能ページ再設計（人物/アルバム/インポート/設定/ツール/地図）→ 6テーマ
