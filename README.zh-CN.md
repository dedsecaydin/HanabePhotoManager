...
- **日期日历**：按拍摄日期快速定位
- **Ctrl+滚轮** 即时缩放缩略图
- 多选模式采用深色勾选反馈，滚动条显示可视比例进度，并通过最小窗口尺寸保护三栏布局
...
### 🧰 批量工具
- **图片压缩** / **拼图** / **水印** 批量处理
- 可直接在真实图片上拖动水印；缩放签名保持中心不变，铺满水印支持高密度覆盖
- 图片长宽比不一致时，可选用当前位置图片的虚化玻璃背景填补拼图留白
...
### 🎬 视频预览
- 基于 LibVLC 的本地视频播放，后台预热引擎并在窗口内复用播放器，缩短首次打开等待
- 支持硬件解码、本地文件缓存、迟帧处理、快进、倍速与全屏
...
- **测试驱动**：647 项自动化测试护航（设计系统回归测试会自动抓出硬编码颜色、圆角越界等 UI 违禁项）
- **迭代节奏**：20% → 60% → 70% → 功能页重设计（人物/相册/导入/设置/工具/地图）→ 6 套主题
