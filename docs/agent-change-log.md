...
## 2026-09-07 — 页面伸展、圆角与日期筛选 UI 审查

- 页面根容器与 `Layout.PagePanel` 增加伸展对齐，修复人物页在宽窗口中只占左侧、浏览条件区未利用可用宽度的问题。
- 主页最近文件夹改为四列自适应网格，人物相册改为六列自适应网格，减少大面积空白并保持卡片间距。
- 恢复页日期筛选改为统一高度的日期 + 时间组合，日期输入复用 `Input.TextBox` 视觉资源，未知时间开关与输入区底部对齐。
- 验证：Release 构建 0 警告/0 错误；全量测试 792 项通过。
...
- 导入页的第二个来源入口由“选择多个文件”改为 Windows 原生“选择多个文件夹”；所选文件夹会作为同一批来源进行后续分析与导入。
- 导入拖放区明确只接收一个或多个文件夹，拖入单个文件时不会启动导入，并提示用户使用文件夹来源。
- Release 构建 0 警告、0 错误；Core 159、Infrastructure 54、App 410、InstallerShell 12，共 635 项测试通过。
...
### Infrastructure 层注释与等价精简

- 为 Infrastructure 的相册存储、文件校验/传输、重复扫描、资源管理、重编号和语义索引共 15 个生产文件补充中文契约与副作用批注。
- 展开语义搜索与分词器中的压缩式单行分支，复用模型缺失消息，并明确临时文件发布、源文件租约、删除前复验和 SQLite 串行访问原因。
- 公共 API 签名集合与基线一致；Infrastructure Release 构建 0 警告、0 错误，55 项测试全部通过。

### App 服务、模型与功能协作者注释

- 为 App 中相册、浏览策略、压缩、重复处理、导入、地图、人物、人脸、元数据、导航、搜索、主题、水印等 73 个非代码后置生产文件补充中文类型职责注释。
- 注释明确了设置兼容、缓存/检查点、文件安全、主题扩散、人脸模型身份、照片库增量扫描和批处理结果等维护边界；未修改 Binding、Command 或业务结果。
- 公共 API 签名集合与基线一致；App Release 构建 0 警告、0 错误，410 项测试全部通过。

### App ViewModel 注释与维护边界

- 为主窗口、导入、压缩、人脸搜索、地图照片、人物相册、照片分析、查看器和标签管理 ViewModel 补充中文职责、状态流与副作用说明。
- 明确主窗口 partial 文件的职责边界、异步请求“最后一次请求生效”策略、缩略图缓存与 RAW/视频预览回退原因；未改变命令、绑定、公开 API 或交互结果。
- 公共 API 签名集合与基线一致；App Release 构建 0 警告、0 错误，410 项测试全部通过。

### WPF 视图适配层与自绘图库注释

- 为 16 个 XAML 代码后置类型补充中文职责说明，明确业务状态与必须依赖 WPF 视觉树、输入设备、系统对话框的视图逻辑边界。
- 为自绘空间树控件补充缓存、语义缩放、可见项调度和命中区域说明；展开水印页面的拖放分支，保持 Shift+单 PNG 更换水印及普通拖入加入输入列表的原有行为。
- XAML Binding、Command、事件处理器和资源键契约与基线一致；App Release 构建 0 警告、0 错误，410 项测试全部通过。
...
## 2026-08-29 — 导入前日期文件夹批量确认

- “开始分析与导入”改为先分析、再暂停在批量确认页面；确认前不执行媒体复制或移动。
- 每个目标日期显示媒体数量、同日期旧文件夹、备注、三种明确策略和最终目录预览；单个旧目录自动选中，修改备注策略自动回填旧备注。
- 三种策略为：新建同日期不同备注目录、重命名所选旧目录后导入、沿用所选旧目录原名导入；未选择、目标重名、源消失等状态会阻止开始导入。
...
## 2026-08-29 — 中断导入续传交互与恢复修复

- 用应用内主题窗口替换启动时的系统 MessageBox，明确提供“继续传输”“放弃记录”“稍后处理”，并说明放弃记录不会删除已传文件。
- 点击继续后先进入导入页，再显示恢复、核对文件与实际导入进度，避免留在浏览页造成无响应错觉。
- 恢复时复用快照中的已确认目标目录；兼容缺少目标目录的旧快照，仅在同日期目录唯一时安全迁移，歧义或目录缺失时保留记录并显示原因。
- 同一日期/目标目录只解析一次，文件核对每 100 项让出 UI 线程，避免大型 UNC 续传记录因重复网络探测或界面不刷新而表现为卡死。
- 新增 4 项回归测试；Release `/warnaserror` 构建 0 警告、0 错误，全量测试 Core 162、Infrastructure 55、App 472、InstallerShell 12，共 701 项通过。

## 2026-08-30 — Hanabe 任务助手拖动与虚线清理

- Hanabe 进度/传输助手卡片支持按住任意空白区域拖动，并通过鼠标捕获保证拖出卡片范围时仍能连续移动；位置被约束在主内容区域内，窗口尺寸变化后会自动重新收束。
- 显式关闭助手内部布局调试线、焦点视觉和进度条焦点框，使用不透明主题表面覆盖，移除卡片上横纵虚线。
- 新增 XAML 交互契约回归断言；Release `/warnaserror` 构建 0 警告、0 错误，全量测试 Core 162、Infrastructure 55、App 476、InstallerShell 12，共 705 项通过。

## 2026-08-30 — 最小化 Q 版助手与来源备注查重

- 移除主窗口内右下角助手卡片；主窗口仅在最小化时创建独立、置顶、可拖动的 Hanabe 任务助手，恢复后隐藏，退出时关闭。Q 版角色素材补入 WPF Resource，修复运行时只显示空圆的问题。
- 新增版本化 Hanabe 备注编解码和 Windows Property System `System.Comment` 读写；保留用户原备注，写入后重新打开属性存储并验证。JPEG 临时副本实测写入与回读成功。
- 导入前计算来源 SHA-256；验证传输成功后把原始文件名、长度和原始哈希写入目标备注。同日期再次导入先按来源备注匹配名称/长度/哈希，再回退现有大小预筛和当前文件 SHA-256。
- Windows 属性处理器不支持、文件只读或无权限时不会强制改写媒体，导入报告会显示备注写入失败并保留旧式查重。
- Release `/warnaserror` 构建 0 警告、0 错误；全量测试 Core 162、Infrastructure 55、App 491、InstallerShell 12，共 720 项通过。自包含发布后运行探测确认：最小化出现“Hanabe 任务助手”，恢复主窗口后收回；本地安装 DLL 与新发布产物哈希一致。

## 2026-08-31 — 双画风动态 Hanabe 助手

- 新增 Q 版动态、8-bit 像素动态、静态和关闭四种持久化角色设置；旧版关闭开关迁移时保持关闭，不会被新默认值重新开启。
- 生成 Q 版六状态精灵表并切分为 6 个透明 GIF；像素图像服务因网络错误未返回，使用同一动作帧进行低色深最近邻像素化，确保动作与角色轮廓一致。最终两套共 12 个 GIF，约 1.30 MiB，全部嵌入 WPF Resource。
...
- 重复内容扫描在主窗口底栏和悬浮助手同步显示扫描阶段、已处理/总数、当前文件、已发现重复组数及预计剩余时间。
- SHA-256 精确查重由逐文件串行改为 2–4 路受控并行读取，在 SSD/多核设备上加快大批量扫描，同时继续以文件大小与完整 SHA-256 双重条件判定重复。
- 主页“最近照片”改为日期文件夹统计卡片，展示封面、照片/视频数量与容量，并可进入对应日期；悬浮助手空闲时隐藏统计详情和进度条。
...
## 2026-09-01 — 悬浮助手空闲形态与原生动作

- 空闲状态彻底折叠任务统计、恢复/处理进度条和停止/继续/查看按钮，窗口收缩为透明背景的小人形态。
- 小人旁新增紧凑对话气泡，按随机间隔显示睡觉、待命和休息提示；使用 WPF 原生位移与轻摆动画赋予角色呼吸感，不再依赖 GIF 本身承担全部动作。
- 只有导入、恢复扫描或图片处理确实运行时才展开任务卡；任务结束后自动恢复小人形态。

## 2026-09-01 — 修复悬浮助手重复异常提示

- 根据 `unhandled-exceptions.log` 定位到悬浮窗成功、失败、跳过计数的 `Run.Text` 默认反写只读属性，窗口布局时会连续抛出三个 UI 异常。
- 三个计数绑定全部明确设为 `Mode=OneWay`，并增加源码回归断言，避免悬浮窗再次触发通用未处理异常弹窗。

## 2026-09-01 — 放大角色与点击动作

- 悬浮角色由 64×72 放大到 96×112，空闲窗口同步扩展，完整全身形象更易辨认。
- 单击角色按顺序切换漂浮、蹦跳、歪头和睡觉四种 WPF 原生动作；延迟判定单击，避免双击时误切动作。
- 双击角色直接恢复并激活 Hanabe 主窗口；人物区域不再误触窗口拖动。

## 2026-09-01 — 图库去重入口与悬浮角色尺寸

- 照片图库页标题区右上角新增固定“扫描重复内容”入口，并在同一区域显示扫描状态，不再要求用户进入设置页寻找。
- 悬浮人物使用系统拖动阈值区分点击与拖动：轻点切动作，双击打开应用，按住移动则拖动窗口。
- 设置 → 外观的角色助手区域新增 64–160 像素尺寸滑杆，按 8 像素步进并持久化；悬浮窗随角色尺寸自适应。

## 2026-09-01 — 多显示器最大化与图库入口位置修复

- 修复自定义无边框窗口在负坐标副屏最大化时使用绝对工作区坐标，导致窗口尺寸或位置越出显示器的问题；最大化位置现在相对当前显示器边界计算。
- 将误放在导入页的“扫描重复内容”工具移到照片图库页标题区右上角。

## 2026-09-01 — 保留悬浮角色手动位置

- 修复悬浮窗在用户拖动后仍被任务状态或尺寸更新重新吸附到右下角的问题。
- 首次显示继续自动靠右下；一旦用户拖动人物或任务卡，当前会话内后续布局更新不再覆盖手动位置。

## 2026-09-02 — 主页最近文件夹强化

- 主页最近内容继续按实际日期目录分组，并强化为明确的文件夹卡片：文件夹名作为主标题，文件夹页签显示完整拍摄日期。
- 每张卡片保留照片数、视频数、容量和少量封面预览，入口文案改为“查看这次拍摄”，便于快速回顾近期工作。

## 2026-09-02 — 主页最近文件夹卡片重绘

- 移除文件夹页签、错位叠图和重复的文字入口，降低单张卡片的视觉噪声。
- 改为一体式三图封面，日期以轻量浮层标记；底部只保留文件夹名、媒体统计和进入箭头。

## 2026-09-02 — 修复最近文件夹卡片运行时崩溃

- 修复日期标签误用不存在的 `Radius.Pill` 资源，导致主页卡片实例化时持续弹出未处理异常并最终触发堆栈溢出的问题。
- 改用设计系统已有的 `Radius.Full`，并增加令牌存在性回归断言。

## 2026-09-06 — 导入恢复、增量查重与设置中心强化

- 导入中断会安全保留恢复点；恢复点保存冻结的目标路径、命名方案、逐组计划和已校验文件凭据，恢复时不重新编号。移动模式在来源删除前先持久化校验凭据，缺失来源只有在目标内容重新核对一致时才视为已完成。
- 导入页新增“继续未完成导入”和“放弃恢复记录”入口，启动提示可在设置中关闭；失败项和来源暂时不可访问项会保留重试。
- SHA-256 与 8×8 视觉指纹改用增量持久缓存，按路径、大小、创建时间和修改时间判断是否复用；新增并行度、缓存开关和清空入口。传输校验和删除前验证仍实时计算。
- 相似复核窗口新增并排图片预览、亮暗网格差异标注、实际一致率和算法局限说明；视觉相似项默认保留，避免未经人工确认直接删除。
- 设置中心新增恢复、缓存、扫描并行度、视觉阈值和差异标注的详细控制，并补充影响说明。
- 验证：Release 构建 0 警告/0 错误；全量测试 782 项通过（Core 166、Infrastructure 64、App 540、Installer 12）。

## 2026-09-06 — 事件音效与设置中心分类
...
## 2026-09-07 — 恢复助手写入时间筛选

- 增加 exFAT 目录读取与本地最后写入时间范围定位，包含目录保留的删除记录和可选未知时间项；保留完整扫描入口。
- 修正空白状态、旧候选复用与导出取消残留，补充候选证据和结果目录入口。
- 验证：Release 构建通过，792 项测试通过，包括日期范围、未知时间、镜像变化及取消清理；真实损坏卡未验证。

## 2026-09-07 — 恢复页内容布局优化

- 镜像来源与时间范围分组、按需展开日期输入、扫描按钮随范围呈现；候选列表改为中文状态和大小摘要，保留右侧证据详情。
- Release 构建和 792 项测试通过。
