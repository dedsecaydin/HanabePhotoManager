...
## 2026-09-12 — 直接设备扫描与自动恢复目录

- 新增可移动存储卡/U 盘的卷级只读扫描入口；复用媒体结构解析，不先制作镜像。独立提权进程只传输读取数据，普通主进程负责输出。
- 自动输出至 `D:\HanabeRecovery\日期\时间_来源_任务编号`；扫描前检查可写性，导出前检查空间、链接重定向和设备物理盘分离。
- 限制：仅有盘符的可移动卷；不支持固定 USB 硬盘、无盘符设备、丢失分区或碎片重组。设备候选暂不在扫描页解码预览，需导出后打开。
- 真实存储卡、UAC 和拔插交互尚未实机验证；本次不宣称实际误删文件恢复率。
- 验证：Release 全解决方案零警告/零错误；819 项测试通过（Core 166、Infrastructure 64、App 577、Installer 12）。独立发布后的读取进程通过固定磁盘拒绝与退出冒烟检查。运行文件在 `.artifacts/direct-recovery-publish`，未覆盖日用程序或发布安装包。

## 2026-09-08 — 增强批次进行中

- 人物页已加入照片选择、移出误识别、拆分为新人物、合并后最近一次整理撤销；操作只修改人物索引引用，不删除原始照片。
- 恢复页已加入安全候选的只读预览、尺寸/解码状态和恢复文件打开入口；RAW 仍以结构证据和原始字节导出为主。
- 查重清理已改为隔离区记录与恢复窗口，保留原路径，冲突时不覆盖；照片库健康检查服务可报告缺失、空文件、图像解码失败和 RAW/JPEG 配对提示。
- 统一任务状态模型已加入任务名、进度、详情、耗时、取消和继续入口的基础组件，尚待主工作区视觉入口整合。

## 2026-09-08 — 视频配套封面与设备导入提醒

- Sony CxxxxTxx.JPG 与同日期 Cxxxx 视频关联，照片墙与筛选数据排除配套 JPG，视频优先读取 T01 封面；无对应视频则保留图片。
- 应用运行时每 5 秒后台检测可移动磁盘，发现非空媒体文件后询问是否进入导入页，默认目标为当前照片库；关闭设置后停止检测。限盘符设备，不含 MTP 或被 Windows 标记为固定磁盘的 USB 硬盘。
- 后续仍需完善：恢复解码预览、统一任务中心、人物拆分撤销、相似差异标注、照片库健康报告和隔离区恢复；本批不宣称这些工作已完成。

## 2026-09-07 恢复页 UI alpha.25

- 镜像来源、查找范围、候选列表和证据面板已重新组织，日期时间按需展开，候选使用中文状态和大小摘要。
- 验证：Release 构建与 792 项测试通过；已查看深色空白状态截图，其他交互视觉状态及真实损坏卡恢复效果未验证。

## 2026-09-07 全页面功能说明入口

- 所有主功能页均可从右下角“本页说明”查看当前页面用途、操作顺序和安全提示。
- 说明会随页面切换自动更新，并同步维护 `docs/user-guide.md`。

## 2026-09-07 页面伸展与日期筛选 UI

- 主页面容器统一使用伸展对齐；主页最近文件夹、人物相册和浏览条件按可用宽度排列。
- 恢复页日期时间筛选已统一控件高度与对齐方式，日期输入与时间输入视觉一致。

## 2026-09-07 恢复助手 alpha.24

- 已实现：exFAT 目录最后写入时间范围扫描、未知时间项开关、候选统计与结构证据、隔离导出及取消清理。
- 时间采用存储卡记录的本地时间，非 MP4 创建时间；目录已丢失的文件需完整扫描，碎片重组仍不支持。
- 自动验证：792 项测试通过，Release 构建无警告与错误。真实受损卡效果未验证。

## 2026-09-06 导入恢复与增量查重

- 已实现并通过自动测试：冻结导入计划、安全恢复凭据、继续/放弃入口、持久指纹缓存及相关设置。
- Implemented-Unverified：相似图片并排预览、网格差异与说明已实现；完整窗口交互仍待验收。
- In Progress：新事件提示音、逐项开关、设置分类及二级菜单内滚动锚点。
- 本阶段验证：Release 构建成功，782 项测试通过。日用快捷方式对应目录已完成本地覆盖并保留旧版备份；正式新版本号待后续工作完成后更新。

## 2026-09-06 音效与设置分类更新

- Stable：开始/恢复、完成、失败、跳过、功能打开、安全停止六类声音均有独立开关；Audacity 本地制作管线已验证导出 24 个 WAV。
- Stable：设置中心新增 AI 与人物、工具默认值分类；压缩/拼图/水印默认值持久化，右侧目录定位到同一滚动页内的分组。
- Verification：构建与全量测试通过；音效资源重新打包前仍需完成正式候选版本发布和日用快捷方式覆盖。

> **Current Version:** `0.3.2-alpha.25`（恢复时间范围与界面优化）

## 2026-08-31 Hanabe 状态音效与重复清理报告

- Hanabe 增加 A 相机机械、B Q 版提示、C 混合三套原创状态音效，覆盖扫描、校验、导入、完成和错误；空闲与逐文件进度不播放。
- 设置中心新增音效开关、风格、音量、安静模式、三种试听，以及“重复检测完成且需要处理时唤回主窗口”。
- 重复检测需要处理时可从最小化状态恢复主窗口；重复清理完成后显示独立报告弹窗，列出成功、跳过、失败和释放空间。
- 音效播放失败与重复报告 UI 不影响导入、扫描、哈希校验和视频播放器音量。

## 2026-08-31 双画风动态 Hanabe 助手

- 最小化助手支持 Q 版动态、8-bit 像素动态、静态形象和关闭四种持久化设置。
- 两套画风均包含待机、扫描、校验、导入、完成和错误 6 个离线 GIF；任务状态结构化驱动切换，完成/错误 3 秒后回到待机。
- 自研 WPF GIF 控件按帧延迟播放，窗口恢复或控件不可见时暂停；像素版使用最近邻缩放，资源异常时回退静态 Q 版图。

## 2026-08-30 Hanabe 最小化助手与导入来源备注

- 主窗口打开时不再显示右下角助手卡片；最小化后显示独立、置顶、可拖动的 Hanabe Q 版任务助手，恢复窗口时自动收回，并继续受设置开关控制。
- Q 版角色 PNG 已作为 WPF Resource 嵌入发布产物，避免运行时退化为空白主题色圆形。
- 新导入的主媒体会把原始文件名、原始长度和写备注前 SHA-256 写入 Windows“备注”；同日期后续导入先匹配来源身份，再以原始 SHA-256 确认，旧文件和不支持写备注的格式继续使用现有查重流程。
- 写备注失败不会让已验证导入失败，报告会列出具体文件和原因；用户已有备注会被保留。
...
## 导入前日期文件夹批量确认（2026-08-29）

| 功能 | 状态 | 说明 |
|---|---|---|
| 分析后确认门禁 | Stable | 来源分析完成后暂停；未确认前不复制、不移动媒体 |
| 日期批量浏览与备注 | Stable | 同页滚动展示全部日期、媒体数量、备注和最终目录预览 |
| 既有目录三策略 | Stable | 新建不同备注、修改原目录备注、沿用原目录名均显式选择 |
| 冻结目标与续传 | Stable | 查重、计划、复制和恢复共用已确认的绝对日期目录 |

Release `/warnaserror` 构建 0 警告、0 错误；Core 162、Infrastructure 55、App 464、InstallerShell 12，共 693 项自动化测试通过。独立日期文件夹管理页继续保留。

## 重复合并检测范围（2026-08-29）

启动全库重复扫描前会询问本次范围：仅 SHA-256 完全相同、仅视觉相似、或两者都包含。默认“两者都包含”保持旧行为；选择范围只生成复查候选，不自动删除、覆盖或移动文件。视觉相似继续使用 64 位 dHash，汉明距离阈值为 8。
...
## 2026-08-31 Hanabe 悬浮窗任务状态闭环

- **状态：** 已实现，自动化验证通过；真人运行时视觉 QA 待发布产物检查。
- **行为：** 悬浮窗使用独立结构化展示快照，区分扫描、校验、传输、安全停止、已取消、意外中断、等待恢复、恢复中、完成、完成但有问题与失败；取消不再显示为错误。
- **信息与操作：** 按状态显示确定/不确定进度、当前阶段、成功/跳过/失败摘要，以及停止、继续、报告和打开主窗口操作。按钮点击不会触发窗口拖动。
- **兼容：** 保留仅主窗口最小化时显示、恢复即隐藏、四种角色样式和缺失动画静态回退；音效问题继续暂缓。

## 2026-08-31 相机视频安全恢复助手

| 功能 | 状态 | 说明 |
|---|---|---|
| 已有镜像只读扫描 | 已实现 | 支持 `.img` / `.raw`，识别 exFAT 并对 MP4 box 做大小、偏移和数量边界检查 |
| 安全直恢 | 已实现 | 仅完整 ftyp/mdat/moov/sample tables 候选；永久保留 raw candidate，生成 MP4、SHA-256、JSON 和 Markdown 报告 |
| 实验候选隔离 | 已实现 | 不完整候选仅展示为 EXPERIMENTAL，不提供默认恢复入口 |
| PhysicalDrive 镜像器 | 计划中 | 尚未暴露；必须独立提权进程并完成系统盘 hard block、源盘输出拒绝和重连协议后才能启用 |
| 导入日期处理默认值 | 已实现 | 已有日期文件夹默认选择第三项“使用原文件夹名，不更改”，多个候选默认第一项，可调整例外 |
